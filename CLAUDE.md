...
- **Auto-commits** — a `Stop` hook auto-commits leftover changes. Do not rely on it for message quality: before finishing, commit your work yourself with a descriptive conventional message (per the rule above) so no `chore: auto commit <timestamp>` commits are ever produced.
