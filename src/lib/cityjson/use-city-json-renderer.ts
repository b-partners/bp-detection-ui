...
const buildGeometry = (faces: number[][], vertices: number[][], faceUVs: Array<Array<[number, number]> | null>): THREE.BufferGeometry => {
  ...
  faces.forEach((face, faceIdx) => {
    const triangles = triangulatePolygon(face, vertices);
...
const parseCityJson = (cityJson: CityJsonData) => {
  ...
  const realVertices = applyTransform(cityJson.vertices as number[][], cityJson.transform);

  const sum = realVertices.reduce((acc, [x, y, z]) => [acc[0] + x, acc[1] + y, acc[2] + z], [0, 0, 0]);
  const center = sum.map(v => v / realVertices.length);

  // CityJSON is z-up; three.js is y-up — swap and re-center on the model itself so it
  // sits at the origin regardless of its real-world (very large) coordinates.
  ...
  Object.values(cityJson.CityObjects as Record<string, any>).forEach(obj => {
    ...
    Object.entries(surfaceFaces).forEach(([surfaceType, faces]) => {
      faces.forEach((face, i) => {
        ...
        const material = new THREE.MeshStandardMaterial({
          ...(texture && surfaceType === 'RoofSurface' ? { map: texture } : { color: new THREE.Color(colors[surfaceType as SurfaceType] ?? '#ffffff') }),
          roughness: 0.8,
